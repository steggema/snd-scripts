...
With input from https://github.com/golovart/Napoli_emulsion/blob/main/SND/neutrino_ml/data_process/hit_sandbox.py
'''
...
def get_paths(options):
    '''Get the path to the geofile and the input directory for MC or data.
    Note that for MC, there's one geo file in every subdirectory; for data, there is one in the main directory
    '''
    ...
    return geo_path, mc_dir

def get_geometry(geo_path):
    '''Get the geometry from the geofile'''
...
if __name__ == '__main__':
    ...
    geo_path, mc_dir = get_paths(options)
    geo = get_geometry(geo_path)
